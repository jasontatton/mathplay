import { Vector3D, Body, SimulationStats } from '../types/types';
...
export const calculateStats = (bodies: Body[], G: number, time: number): SimulationStats => {
  // Kinetic energy: 0.5 * m * v^2
  const kineticEnergy = bodies.reduce((sum, body) => {
    return sum + 0.5 * body.mass * magnitudeSquared(body.velocity);
  }, 0);
