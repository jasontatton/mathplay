...
# Used to generate book_metadata_y5.json from the booklist
...
def main():
    # if no isbn in books_y5.txt get most likely isbn from google api
    # when most likely found then suppliment books_y5.txt - if can't find one or the wrong one is picked then we need to
    # # get it manually

    books = []
    with open("./books_y5.txt", 'r') as f:
        for line in f:
            line = line.strip()
    ...
    with open("./books_y5.txt", 'w') as f:
        f.writelines([f"{info['author']}, {info['title']}, {info['isbn']}\n" for info in results])

    print(f"Results written to book_metadata_y5.json and books_y5.txt ({len(results)} books)")
